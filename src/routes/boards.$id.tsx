...
const statusMeta: Record<Status, { label: string; classes: string; icon: LucideIcon }> = {
  ready: { label: "Ready", classes: "bg-success/10 text-success border-success/20", icon: CheckCircle2 },
  "in-progress": { label: "In progress", classes: "bg-info/10 text-info border-info/20", icon: Clock },
  attention: { label: "Needs attention", classes: "bg-warning/10 text-warning border-warning/20", icon: AlertTriangle },
};
...
const dimensions: Dimension[] = [
  {
    ...
    score: 82,
    status: "ready",
    summary: "14 interviews and 2 usability studies show strong pull from mid-market segment.",
    signals: [
  ...
  },
  {
    ...
    score: 64,
    status: "in-progress",
    summary: "Core flows implemented. Latency budget still being tuned in the inference layer.",
    signals: [
  ...
  },
  {
    ...
    score: 71,
    status: "in-progress",
    summary: "Pricing model drafted, awaiting finance sign-off. GTM narrative in review.",
    signals: [
  ...
  },
  {
    ...
    score: 48,
    status: "attention",
    summary: "Support runbooks incomplete; on-call rotation for the new service not staffed.",
    signals: [
...
const activity = [
  { who: "Priya Natarajan", what: "attached the Q3 pricing sensitivity analysis to Business Readiness.", when: "12m ago", icon: FileText },
  { who: "Elliot Chen", what: "commented on the SSO rollout risk in Stakeholder Alignment.", when: "1h ago", icon: MessageSquare },
  { who: "Axiora", what: "flagged missing on-call rotation under Operational Readiness.", when: "3h ago", icon: Sparkles },
  { who: "Jordan Klein", what: "marked Customer Validation as ready for review.", when: "Yesterday", icon: CheckCircle2 },
];
...
function BoardOverview() {
  ...
  return (
    <AppShell title="AI copilot rollout" subtitle={`Decision board · ${id} · owner Maya Ramirez`}>
      <div className="mx-auto max-w-7xl space-y-8">
          ...
          <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_auto] md:items-start">
            <div className="min-w-0">
              ...
              <p className="mt-3 max-w-3xl text-sm text-muted-foreground">
                A cross-functional decision spanning Product, GTM, and Operations. Axiora is preparing
                evidence across five assessment dimensions to help the team decide by{" "}
                <span className="text-foreground">Jan 24, 2027</span>.
              </p>
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-1.5 rounded-md border border-border bg-surface px-3 py-2 text-sm hover:bg-surface-muted"
                >
                  <Sparkles className="h-4 w-4" /> Analysis Results
                </Link>
              </div>
            </div>
            ...
            <div className="flex flex-col items-end gap-3">
              ...
              <div className="flex items-center gap-4 rounded-xl border border-border bg-surface-muted px-4 py-3">
                <div className="text-right">
                  ...
                  <div className={`font-display text-3xl ${scoreColor(overallScore)}`}>{overallScore}</div>
                </div>
        ...
        <section>
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <Link to="/boards/new" className="text-xs font-medium text-accent hover:underline">
              Configure
            </Link>
          </div>
            ...
            {dimensions.map((d) => {
              ...
              return (
                ...
                >
                  ...
                  {d.name === "Customer Validation" ? (
                    <Link
                      to="/boards/$id/customer-validation"
                      params={{ id }}
                      className="mt-5 inline-flex items-center gap-1 self-start text-xs font-medium text-accent opacity-0 transition-opacity group-hover:opacity-100"
                    >
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2 rounded-xl border border-border bg-surface">
            <div className="border-b border-border px-6 py-4">
              ...
              <p className="text-xs text-muted-foreground">Latest updates from your team and Axiora</p>
            </div>
          ...
          <aside className="rounded-xl border border-border bg-primary p-6 text-primary-foreground">
            ...
            <Link
              to="/boards/$id/analysis"
              params={{ id }}
              className="mt-5 inline-flex items-center gap-1.5 rounded-md bg-primary-foreground/10 px-3 py-1.5 text-xs font-medium hover:bg-primary-foreground/20"
            >
